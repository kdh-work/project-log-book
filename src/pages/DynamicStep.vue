<template>
  <div class="p-4 pr-96">
    ...
    <div class="bg-white p-6 rounded-lg shadow">
      ...
      <p class="text-gray-700 mb-6">
        여기에 메인 콘텐츠가 들어갑니다. 오른쪽에 고정된 사이드바가 있습니다. {{ currentStep }}
      </p>
      
      <!-- 디버깅 정보 -->
      <div class="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded">
        <p class="text-sm text-yellow-800">
          <strong>현재 단계:</strong> {{ currentStep }} ({{ stepStore.getCurrentStepName }})<br>
          <strong>컴포넌트:</strong> {{ stepStore.getCurrentStepComponent }}<br>
          <strong>장치 유형:</strong> {{ stepStore.getSelectedDeviceType || '선택되지 않음' }}<br>
      ...
      <div class="step-content-area">
        ...
        <div v-else class="text-center py-8 text-gray-500">
          ...
          <p class="text-sm mt-2">현재 단계: {{ stepStore.getCurrentStepComponent || '로딩 중' }}</p>
        </div>
      </div>
  ...
  <!-- 오른쪽 고정 사이드바 -->
  <div class="fixed right-0 top-0 bottom-0 w-80 bg-white shadow-lg border-l border-gray-200 z-10">
    <div class="p-4 h-full overflow-y-auto">
      ...
      <div v-if="!stepStore.getSelectedDeviceType" class="text-center py-8">
        ...
        <p class="text-sm text-gray-400">Step 1에서 장치 유형을 선택하면 단계가 표시됩니다.</p>
      </div>
      ...
      <div v-else class="space-y-3">
        <div class="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
          ...
          <p class="text-sm text-blue-600">{{ getDeviceTypeName(stepStore.getSelectedDeviceType) }}</p>
        </div>
...
const handleRestartProcess = () => {
  console.log('프로세스를 처음부터 다시 시작합니다.');
  // 모든 단계 초기화
  stepStore.resetAllSteps();
};
